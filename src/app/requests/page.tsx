        ...
        setRequests([
          {
            ...
            description: 'Critical need for antibiotics and IV fluids in emergency ward',
      ...
      <div className="block rounded-lg bg-white p-6 shadow transition-all hover:shadow-lg dark:bg-slate-800">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex gap-2">
            <span className={`rounded-full px-3 py-1 text-sm font-medium ${priorityColors[request.priority]}`}>
              {request.priority}
            </span>
            <span className={`rounded-full px-3 py-1 text-sm font-medium ${statusColors[request.status]}`}>
              {request.status}
            </span>
          </div>
        </div>
